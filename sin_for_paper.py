...
T2 = np.sin(X2)
ax.plot(X2,T2,'b', label=u'função seno')
...
title = ax.set_title("\n".join(wrap(u'Base de treinamento sin(x) com ruído gaussiano', 60)))
title.set_y(1.05)
fig.subplots_adjust(top=0.8)
plt.xlabel('x')
plt.ylabel('sin(x)')
ax.legend(bbox_to_anchor=[1, 1])
fig.savefig("sin_5v1_train_data_paper.png")
...
title = ax.set_title("\n".join(wrap(u'Aproximando função seno com 5 neurônios RBF e variância 1.0 (10mil épocas)', 60)))
title.set_y(1.05)
fig.subplots_adjust(top=0.8)
plt.xlabel('x')
plt.ylabel('sin(x)')
ax.legend(bbox_to_anchor=[1, 1])
fig.savefig("sin_5v1_model_paper.png")
...
plt.ylabel('sin(x)')
ax.legend(bbox_to_anchor=[1, 1])
fig.savefig("sin_5v1_all_paper.png")
