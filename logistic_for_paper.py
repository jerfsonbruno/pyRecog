...
ax.plot(X2,T2,'b', label=u'função logística')
...
title = ax.set_title("\n".join(wrap(u'Aproximando função logística com 5 neurônios RBF e variância 1.0 (10mil épocas)', 60)))
title.set_y(1.05)
fig.subplots_adjust(top=0.8)
plt.xlabel('x')
plt.ylabel('f(x)')
ax.legend(bbox_to_anchor=[1, 0.3])
fig.savefig("f_5v1_model_paper.png")
...
plt.ylabel('f(x)')
ax.legend(bbox_to_anchor=[1, 0.3])
fig.savefig("f_5v1_all_paper.png")
